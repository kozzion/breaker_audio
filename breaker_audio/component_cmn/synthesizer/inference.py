...
from breaker_audio.component_cmn.synthesizer.utils import audio
from breaker_audio.component_cmn.synthesizer.tacotron2 import Tacotron2
from breaker_audio.component_cmn.synthesizer.hparams import hparams as default_hparams
...
class Synthesizer:
    ...
    def __init__(self, checkpoints_dir: Path, verbose=True, low_mem=False):
        ...
        memory is low (<= 2gb)
        """
        path_file_synthesizer_meta = checkpoints_dir.joinpath('hparams.json')
        json_hparams = json.load(open(path_file_synthesizer_meta, encoding="utf8"))
        import aukit
        hparams = aukit.Dict2Obj(json_hparams) #TODO fixe after merge with english
