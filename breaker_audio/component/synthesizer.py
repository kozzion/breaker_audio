...
class Synthesizer:
    ...
    def load_model(self, path_dir_model: Path):
        ...
        self._model = Tacotron(embed_dims=hparams.tts_embed_dims,
                               ...
                               speaker_embedding_size=hparams.speaker_embedding_size).to(self._device)
    ...
    def synthesize_spectrograms(self, texts: List[str],
        ...
        if not self.is_loaded():
            raise Exception('Load before using')
        ...
        for i, batch in enumerate(batched_inputs, 1):
            ...
            # Convert to tensor
            chars = torch.tensor(chars).long().to(self._device)
            speaker_embeddings = torch.tensor(speaker_embeds).float().to(self._device)
